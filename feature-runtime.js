...
function apply(){
  if(!settings)return;
  const className=settings['class.name'];
  if(className){document.documentElement.dataset.className=String(className);document.querySelectorAll('[data-class-name]').forEach(el=>el.textContent=String(className));}
  Object.entries(mapViewToKey).forEach(([view,key])=>{
    document.querySelectorAll(`[data-nav="${view}"]`).forEach(el=>{
      el.hidden=!enabled(key);el.setAttribute('aria-hidden',enabled(key)?'false':'true');
    });
  });
