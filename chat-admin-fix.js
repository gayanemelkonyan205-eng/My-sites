...
function toast(text,kind=''){const h=$('#toast');if(!h)return;const e=document.createElement('div');e.className=`toast ${kind}`;e.textContent=text;h.append(e);setTimeout(()=>e.remove(),4200)}
...
async function renderAdminChat(){
  ...
  body.innerHTML=`<div class="cc-grid cc-grid-2 cc-top-align">
    ...
    <article class="cc-card"><div class="cc-section-head"><div><h3>Չատի մոդերացիա</h3><p class="cc-muted">Ջնջում, վերականգնում, ամրացում և unpin։</p></div><select id="ccaf-conv">${list.map(c=>`<option value="${c.id}">${c.type==='CLASS'?'🏫':c.type==='DIRECT'?'👤':'👥'} ${esc(c.title||c.type)}</option>`).join('')}</select></div><div id="ccaf-pins"></div><div id="ccaf-messages" class="cc-mini-messages"></div></article>
  </div>`;
  ...
  async function load(){
    ...
    host.innerHTML=messages.map(m=>{const s=senderMap.get(m.sender_id);const name=s?`${s.first_name||''} ${s.last_name||''}`.trim():'Մասնակից';return `<div class="cc-message ${m.deleted_at?'deleted':''}"><div><b>${esc(name)}</b><span>${esc(m.body||'')}</span><small>${fmt(m.created_at)}${pinned.has(m.id)?' · 📌':''}</small></div><div>${m.deleted_at&&profile.role==='SUPER_ADMIN'?`<button data-restore="${m.id}">Վերականգնել</button>`:!m.deleted_at?`${pinned.has(m.id)?`<button data-unpin="${m.id}">Unpin</button>`:`<button data-pin="${m.id}">📌</button>`}<button class="cc-danger-mini" data-delete-message="${m.id}">Ջնջել</button>`:''}</div></div>`}).join('')||'<div class="cc-empty">Հաղորդագրություններ չկան</div>';

    $$('[data-delete-message]',body).forEach(b=>b.onclick=async()=>{const r=await sb.rpc('admin_delete_message',{p_message_id:b.dataset.deleteMessage});if(r.error||!r.data)return toast(r.error?.message||'Չհաջողվեց','err');track('message_moderated');await load()});
